...
- **Intent-Based Routing** based on semantic understanding of user queries
- **Embedding-Powered Matching** using vector embeddings to compute similarity between user requests and predefined utterances
- **Confidence Scoring** with configurable similarity thresholds
- **Default Fallback** to a configured model when no semantic match is found
...
- Embedding provider configured in `deployment.toml`
